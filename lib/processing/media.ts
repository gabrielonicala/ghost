...
export async function transcribeAudio(
  ...
  try {
    ...
    if (typeof audioBufferOrUrl === "string") {
      ...
      // YouTube: Use Apify scrapers for transcription
      if (platformInfo?.platform === "youtube") {
        const videoId = extractYouTubeVideoId(audioBufferOrUrl);
        if (videoId) {
          console.log("[Transcribe YouTube] Processing YouTube video:", videoId);
          console.log("[Transcribe YouTube] Apify configured:", isApifyConfigured());
          
          if (!isApifyConfigured()) {
            throw new Error(
              "YouTube transcription requires APIFY_API_TOKEN.\n\n" +
              "Please add APIFY_API_TOKEN to your environment variables (Vercel dashboard)."
            );
          }
          
          // Method 1: Try Apify YouTube Transcript scraper (gets captions + metadata)
          console.log("[Transcribe YouTube] Trying Apify transcript scraper (actor: Uwpce1RSXlrzF6WBA)...");
          try {
            const transcriptResult = await fetchYouTubeTranscript(audioBufferOrUrl);
            
            console.log("[Transcribe YouTube] Transcript scraper result:", {
            ...
            if (transcriptResult?.fullTranscriptText && transcriptResult.fullTranscriptText.trim().length > 0) {
              console.log("[Transcribe YouTube] SUCCESS - Got transcript from Apify scraper:", 
              ...
              console.log("[Transcribe YouTube] Transcript scraper returned empty transcript, trying video download...");
            }
          } catch (apifyError: any) {
            console.error("[Transcribe YouTube] Transcript scraper failed:", apifyError.message);
            console.error("[Transcribe YouTube] Full error:", apifyError);
          }
          
          // Method 2: Download video via Apify and use Whisper
          console.log("[Transcribe YouTube] Trying Apify video downloader (actor: cmC0PdCbAkXY5NRAw)...");
          try {
            const downloadResult = await fetchYouTubeDirectVideoUrl(audioBufferOrUrl);
            
            console.log("[Transcribe YouTube] Video downloader result:", {
              hasResult: !!downloadResult,
              hasDirectUrl: !!downloadResult?.directVideoUrl,
              videoId: downloadResult?.videoId,
              formatsCount: downloadResult?.formats?.length || 0,
            });
            
            if (downloadResult?.directVideoUrl) {
              console.log("[Transcribe YouTube] Got direct video URL, downloading...");
              console.log("[Transcribe YouTube] URL preview:", downloadResult.directVideoUrl.slice(0, 100) + "...");
              
              const videoBuffer = await downloadVideoFile(downloadResult.directVideoUrl);
              console.log("[Transcribe YouTube] Downloaded video:", videoBuffer.length, "bytes");
              console.log("[Transcribe YouTube] Sending to Whisper for transcription...");
              
              // Transcribe with Whisper
              const whisperResult = await transcribeAudio(videoBuffer, language);
              console.log("[Transcribe YouTube] SUCCESS - Whisper transcription complete:", whisperResult.text.slice(0, 200) + "...");
              return whisperResult;
            } else {
              console.error("[Transcribe YouTube] Video downloader returned no direct URL");
            }
          } catch (downloadError: any) {
            console.error("[Transcribe YouTube] Video download/Whisper failed:", downloadError.message);
            console.error("[Transcribe YouTube] Full error:", downloadError);
          }
          
          // Both methods failed
          throw new Error(
            "YouTube video transcription failed.\n\n" +
            "Both Apify scrapers failed:\n" +
            "1. Transcript scraper (Uwpce1RSXlrzF6WBA) - failed or returned empty\n" +
            "2. Video downloader (cmC0PdCbAkXY5NRAw) + Whisper - failed\n\n" +
